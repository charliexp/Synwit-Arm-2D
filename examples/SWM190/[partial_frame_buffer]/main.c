...
void display_task(void)
{
    ...
    ADD_REGION_TO_LIST(s_tDirtyRegions,
        ...
        ADD_LAST_REGION_TO_LIST(s_tDirtyRegions, .tLocation = {0, APP_SCREEN_HEIGHT - 16}, .tSize = {
...
void example_gui_on_refresh_evt_handler(const arm_2d_tile_t *ptFrameBuffer)
{
    ...
    if (0 == BENCHMARK.wIterations)
    {
        ...
        lcd_puts("Partial Framebuffer Test, running " STR(ITERATION_CNT) " iterations\r\n");

        lcd_puts("PFB Size: " STR(PFB_BLOCK_WIDTH) "*" STR(PFB_BLOCK_HEIGHT) "  Screen Size: " STR(APP_SCREEN_WIDTH) "*" STR(APP_SCREEN_HEIGHT));
